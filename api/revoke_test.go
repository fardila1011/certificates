...
func Test_caHandler_Revoke(t *testing.T) {
	...
	tests := map[string]func(*testing.T) test{
		...
		"200/ott": func(t *testing.T) test {
			...
			return test{
				...
				auth: &mockAuthority{
					...
					revoke: func(ri authority.RevokeRequestInfo) error {
						assert.True(t, ri.PassiveOnly)
						assert.False(t, ri.MTLS)
						assert.Equals(t, ri.Serial, "sn")
						assert.Equals(t, ri.ReasonCode, 4)
						assert.Equals(t, ri.Reason, "foo")
						assert.Equals(t, ri.ProvisionerID, "provisioner-id")
						return nil
					},
				},
		...
		"404/peer certificate load provisioner error": func(t *testing.T) test {
			cs := &tls.ConnectionState{
				PeerCertificates: []*x509.Certificate{parseCertificate(certPEM)},
			}
			input, err := json.Marshal(RevokeRequest{
			...
			return test{
				...
				auth: &mockAuthority{
					loadProvisionerByCertificate: func(crt *x509.Certificate) (provisioner.Interface, error) {
						return nil, &Error{Err: errors.New("force"), Status: http.StatusNotFound}
					},
				},
			}
		},
			...
			return test{
				...
				auth: &mockAuthority{
					revoke: func(ri authority.RevokeRequestInfo) error {
						...
						assert.Equals(t, ri.ReasonCode, 4)
						assert.Equals(t, ri.Reason, "foo")
						assert.Equals(t, ri.ProvisionerID, "mock-provisioner-id")
						return nil
					},
					loadProvisionerByCertificate: func(crt *x509.Certificate) (provisioner.Interface, error) {
