...
func (h *caHandler) Revoke(w http.ResponseWriter, r *http.Request) {
	...
	if len(body.OTT) > 0 {
		...
		p, err := h.Authority.LoadProvisionerByCertificate(clientCrt)
		if err != nil {
			fmt.Printf("err = %+v\n", err)
			WriteError(w, err)
			return
	...
	ri := authority.RevokeRequestInfo{
		Serial:        body.Serial,
	...
	if err := h.Authority.Revoke(ri); err != nil {
...
func logRevoke(w http.ResponseWriter, ri authority.RevokeRequestInfo) {
	if rl, ok := w.(logging.ResponseLogger); ok {
		rl.WithFields(map[string]interface{}{
			"serial":        ri.Serial,
			"provisionerID": ri.ProvisionerID,
